...
# ── HubSpot dedup in Instagram prescreen ────────────────────────────────────

class TestInstagramPrescreenHubspotDedup:
    """HubSpot dedup runs before expensive InsightIQ/GPT calls."""

    @patch('app.services.hubspot.check_existing_contacts')
    @patch('app.services.insightiq.fetch_social_content')
    def test_existing_contacts_skipped_before_api_calls(
        self, mock_fetch, mock_dedup, mock_run,
    ):
        """Profiles matching HubSpot emails skip InsightIQ entirely."""
        mock_dedup.return_value = {'alice@example.com'}

        adapter = InstagramPrescreen()
        profiles = [
            {'url': 'https://instagram.com/alice', 'email': 'alice@example.com'},
            {'url': 'https://instagram.com/bob', 'email': 'bob@example.com'},
        ]
        result = adapter.run(profiles, mock_run)

        # Alice skipped, Bob goes through (and fails on fetch — that's fine)
        assert result.skipped >= 1
        assert any(f['type'] == 'hubspot_duplicate' for f in result.meta.get('filtered', []))
        assert result.meta['hubspot_duplicates'] == 1
    ...
    @patch('app.services.insightiq.filter_content_items', return_value=[{'id': 1}])
    @patch('app.services.insightiq.fetch_social_content')
    def test_non_duplicate_profiles_still_processed(
        self, mock_fetch, mock_filter, mock_travel, mock_freq,
        mock_snapshot, mock_prescreen, mock_dedup, mock_run,
    ):
        """Profiles not in HubSpot go through normal pre-screen flow."""
        mock_dedup.return_value = set()  # No duplicates
        mock_fetch.return_value = {
            'data': [{'id': 1, 'profile': {'platform_username': 'bob', 'follower_count': 50000, 'image_url': ''}}],
        }
        mock_snapshot.return_value = Image.new('RGB', (100, 100), 'white')
        mock_prescreen.return_value = {
            'decision': 'continue', 'reasoning': 'ok',
        ...
        adapter = InstagramPrescreen()
        profiles = [{'url': 'https://instagram.com/bob', 'email': 'bob@example.com', 'bio': '', 'follower_count': 50000}]
        result = adapter.run(profiles, mock_run)

        assert len(result.profiles) == 1
        assert 'hubspot_duplicates' not in result.meta

    @patch('app.services.hubspot.check_existing_contacts')
    @patch('app.services.insightiq.fetch_social_content')
    def test_all_profiles_deduped_returns_empty(
        self, mock_fetch, mock_dedup, mock_run,
    ):
        """When all profiles exist in HubSpot, no API calls are made."""
        mock_dedup.return_value = {'a@b.com', 'c@d.com'}

        adapter = InstagramPrescreen()
        profiles = [
            {'url': 'https://instagram.com/a', 'email': 'a@b.com'},
            {'url': 'https://instagram.com/c', 'email': 'c@d.com'},
        ]
        result = adapter.run(profiles, mock_run)
        ...
        assert result.skipped == 2
        assert result.processed == 2
        assert result.meta['hubspot_duplicates'] == 2
        # InsightIQ never called
    ...
    @patch('app.services.hubspot.check_existing_contacts')
    @patch('app.services.insightiq.fetch_social_content')
    def test_profiles_without_email_not_deduped(
        self, mock_fetch, mock_dedup, mock_run,
    ):
        """Profiles missing email field are never marked as duplicates."""
        mock_dedup.return_value = set()
        mock_fetch.return_value = {'data': []}

        adapter = InstagramPrescreen()
        profiles = [
            {'url': 'https://instagram.com/noemail'},
        ]
        result = adapter.run(profiles, mock_run)

        # Profile goes through to pre-screen (filtered for no_content, but not hubspot_duplicate)
        hubspot_dupes = [f for f in result.meta.get('filtered', []) if f['type'] == 'hubspot_duplicate']
        assert len(hubspot_dupes) == 0
